...
import { showModule as showModuleDefault, clearModule as clearModuleDefault } from "./moduleDisplayManager";
...
export default class ModuleScheduler {
  ...
  private initModules(modules: ModuleProps[]): void {
    ...
    console.log("[ModuleScheduler] initModules — rotating:", this.rotatingModules.map(m => m.type), "interval:", this.intervalModules.map(m => m.type));
  }
  ...
  private run(module: ModuleProps): void {
    ...
    this.deps.showModule({
      ...module,
      onShutdownRequest: (trigger) => { this.shutdownTrigger = trigger; },
      onModuleDone: () => { this.handleModuleDone(); },
    });

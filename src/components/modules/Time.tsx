...
function DigitalClock({ now, format = "HH:mm", showSeconds = false }: { now: Date; format?: string; showSeconds?: boolean }) {
  ...
  return (
    <Box sx={{ width: "min(65vh, 65vw)", aspectRatio: "1 / 1" }}>
      <svg viewBox="0 0 200 200" style={{ width: "100%", height: "100%" }}>
        ...
        {showSeconds && (
          <line
            x1={cx} y1={cy} x2={second.x} y2={second.y}
  ...
  useEffect(() => {
    onShutdownRequest?.(() => {
      if (window.speechSynthesis.speaking) {
        window.speechSynthesis.addEventListener("end", () => onModuleDone?.(), { once: true });
      } else {
        onModuleDone?.();
      }
    });
